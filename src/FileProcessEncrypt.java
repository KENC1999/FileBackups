...
public class FileProcessEncrypt {
    ...
    public static void writeHead(String outPath,Map<Byte,byte[]> codeMap,String passwd){
        System.out.print("3.写入文件头信息：");
        try {
            FileOutputStream fileOutputStream=new FileOutputStream(outPath);

            //System.out.print("是否需要加密？");
            ...
            if(passwd.length()!=0&&passwd!=""){
                ...
                String pd = passwd;
                byte[] password = pd.getBytes();
                System.out.println(passwd);
                System.out.println(Arrays.toString(password));
                pdlength = pd.getBytes().length;
                //System.out.println(pdlength);
                fileOutputStream.write((byte)pdlength);
    ...
    //压缩操作
    public static void comPress(String path,String outpath,String passwd){
        ...
        writeHead(outPath,codeMap,passwd);
    ...
    public static boolean unCompress(String path,String name){
        ...
        try {
            ...
            fis.read(paslength);
            if(paslength[1] != 0) {
                byte[] jiami = new byte[paslength[1]];
                fis.read(jiami);
                //System.out.println("此文件已加密，请输入密码：");
                @SuppressWarnings("resource")
                //Scanner sc = new Scanner(System.in);
                //String pd = sc.next();
                ...
                label.setFont(font);
                panel.add(label);
                JPasswordField pwdField = new JPasswordField(20);pwdField.setEchoChar('*');
                pwdField.setFont(font);
                panel.add(pwdField);
                int result=JOptionPane.showOptionDialog(null, panel, "文件已加密",
                        JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null,
                        options,null);
                if(result!=0)
                    return false;
                String pd =pwdField.getText();
                String pass = new String(jiami);
                System.out.println(pd+"------"+pass);
                if(!pd.equals(pass)) {
            ...
            fis.read(littleBytes2);
            littleBytes[0] = paslength[0];
            littleBytes[1] = littleBytes2[1];
            littleBytes[2] = littleBytes2[2];
            //System.exit(0);
    ...
    //多线程压缩操作
    public static  void mutlComPress(String path,String passwd) {
